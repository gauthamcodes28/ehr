...
if(isset($_POST['order_type']))
{
	if($_POST['order_type']=='viewmedicine')
	{
		...
		$sql="SELECT distinct p.Patient_ID,p.Visit_Number,p.Medicine_Type_ID FROM patient_medicine p where p.Patient_ID LIKE '".$patient_id."' and p.Visit_Number=".$v_id."";
		...
		{
			$pid[$i]=$row['Patient_ID'];
			$visit[$i]=$row['Visit_Number'];

			$tid[$i]=$row['Medicine_Type_ID'];

			$s="select Type_Name from medicine_type where Type_ID=".$row['Medicine_Type_ID'];
			$r=mysqli_query($conn,$s);
			$rw=mysqli_fetch_assoc($r);
			$tname[$i]=$rw['Type_Name'];
			$i++;
		}
		$data['pid']=$pid;
		$data['visit']=$visit;
		...
		$sql="select *  from patient_medicine p join medicine m,medicine_type t where p.Patient_ID like '".$patient_id."' and  p.Visit_Number=".$v_id." and p.Medicine_Name=m.Medicine_Name and p.Medicine_Type_ID=t.Type_ID AND p.Medicine_Type_ID NOT IN (3,4) order by p.Visit_Number asc";
		$result=mysqli_query($conn,$sql);
	...
	if($_POST['order_type']=='newpatientsave')
	{
...
if(!mysqli_num_rows($result)==0)
	...
	if($_POST['order_type']=='updatesave')
	{
		//$target_dir = "images/";
	...
	/* else
		echo "No New File Updated"; */

	
	// $sql="delete from patient_medicine where Patient_ID='".$patient_id."'  and Visit_Number=".$visit;
	// mysqli_query($conn,$sql);
...
//Medicine Details Entry
		...
		for($i=0;$i<sizeof($type);$i++){
			...
			if(empty($ai[$i]))
				...
				$sql="CALL patient_medicine_data('$patient_id',".$visit.",".$row2['Type_ID'].",'".$name[$i]."','".$billable[$i]."','".$given[$i]."','".$quantity[$i]."','".$whenval[$i]."','".$how[$i]."','update',".$ai[$i].");"; 

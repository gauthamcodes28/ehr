...
//Note:----
//scripts will be there in newform.js in scripts folder..

//star of json file generation code
// function get_data ()//for all the medicines in the db
// {
// 	include '../07config/newdbconfig.php';
// 	$query = "SELECT * FROM medicine m join medicine_type t where m.Medicine_Type_ID=t.Type_ID";
...
// 	return json_encode($medicine_type);
// }
...
// }
//  */
// //End of JSON FILE GENERATION CODE... 


// //all medicine details of the selected patient
// function pmed_data($id)
// {
	
// 		include '../07config/newdbconfig.php';
		
// 		$sql="select *  from patient_medicine p join medicine m,medicine_type t where p.Patient_ID like '".$id."'  and p.Medicine_Name=m.Medicine_Name and p.Medicine_Type_ID=t.Type_ID AND p.Medicine_Type_ID NOT IN (3,4) order by p.Visit_Number asc";
...
// //only the type names and the options of the medicines selected for the particular  patient

// function pmed_types($id)//to know only the types selected..
// {
// include '../07config/newdbconfig.php';

// 		$sql="SELECT distinct p.Patient_ID,p.Visit_Number,p.Medicine_Type_ID FROM patient_medicine p where p.Patient_ID LIKE '".$id."' AND p.Medicine_Type_ID NOT IN (3,4);";
...
// 		{
// 			$select_values=array();
// 			$sql2="select * from medicine m where m.Medicine_Type_ID=".$row['Medicine_Type_ID'];
// 			$result2=mysqli_query($conn,$sql2);
// 			while($row2=mysqli_fetch_assoc($result2))
// 			{
// 				$select_values[]=$row2['Medicine_Name'];
// 			}

// 			//Type Name get
// 			$sql3 = "select Type_Name from medicine_type where Type_ID = ".$row['Medicine_Type_ID'];
...
// function medicine_types()//To store all the data of the medicines types
// {
// 	include '../07config/newdbconfig.php';
		
// 		$sql="select t.Type_ID,t.Type_Name from medicine m join medicine_type t where t.Type_ID=m.Medicine_Type_ID group by t.Type_ID";
// 		$result=mysqli_query($conn,$sql);
// 		$data=array();
// 		//echo $sql;
// 		while($row=mysqli_fetch_assoc($result))
// 		{	
// 			$select_values=array();
// 			$sql2="select * from medicine m where m.Medicine_Type_ID=".$row['Type_ID'];
// 			$result2=mysqli_query($conn,$sql2);
// 			while($row2=mysqli_fetch_assoc($result2))
// 			{
// 				$select_values[]=$row2['Medicine_Name'];
...
// $file_name="./json/medicine_types.json";
// file_put_contents($file_name,medicine_types());
/* if(file_put_contents($file_name,medicine_types()))
...
//View
else if($_GET['status']=='view')
	{
		...
		echo '<script>	
				...
				//echo pmed_data($id,$visit);
				//echo "Hello WOrld".$id.$visit;
				/* if(file_put_contents($file_name, pmed_data($id,$visit)))
					echo "success";
				else
					echo "fail"; */

				// $file_name="./json/pdata.json";//patient medicine
				// file_put_contents($file_name, pmed_data($id));
				
				// $file_name="./json/types.json";
				// file_put_contents($file_name,pmed_types($id));//type names of the patient
...
//Followup
	else if($_GET['status']=='followup')
	{
		...
		echo '<script>$(function(){if(devmode){console.log("follow up mode");}});</script>';
	        ...
	        <ul class=" list-inline"  >
					...
					<li  class="pull-right top-buttons"  id="visit-container">
					 ...
					 <li style="display:none;" class="pull-right top-buttons">
					  ...
					  <li style="display:none;"  class="pull-right top-buttons">
								<button  style="" class="btn btn-primary"  type="button" id="updatesave">save</button>
					  </li>
					  <li style="display:none;" class="pull-right top-buttons">
									<button  style="" class="btn btn-primary"  type="button" id="followsave">Save</button>
					 </li>
					  <li style="display:none;" class="pull-right top-buttons">
									<button  style="" class="btn btn-primary"  type="button" id="newsave">Save</button>
					  </li>	
	      </ul>
	   </div>
   </div>
